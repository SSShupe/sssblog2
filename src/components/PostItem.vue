      ...
      <div class="py-8 sm:py-20 border-b border-gray-300">
        <header class="text-center mb-8">
          <time
            ...
            class="text-blue-600 text-sm mb-2 uppercase"
          >{{ formatPublishDate(post.datetime) }}</time>
          <h2 class="text-3xl sm:text-4xl leading-tight font-serif mb-1 sm:mb-2">
            <g-link :to="`${post.path}/`" class="text-black font-bold">{{ post.title }}</g-link>
          </h2>
          <p class="text-blue-600 leading-normal text-sm sm:text-base">
            <span v-if="post.author">
              by
              <g-link
                :to="`${post.author.path}/`"
                class="text-blue-600 capitalize border-b border-transparent hover:border-gray-400 transition-colors duration-300"
                v-if="post.author"
